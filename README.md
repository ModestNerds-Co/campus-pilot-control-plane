...
The wire contract version (`cp-license/v1`) and product catalog version are independent. The control plane currently signs `catalog_version: campus-pilot/6`, matching the code-owned module, feature, limit, and operation vocabulary in Campus Pilot. Commercial plan keys never appear in this compatibility field. A campus verifies explicit catalog support before persisting a lease or changing module state; the current runtime accepts `campus-pilot/2`, `campus-pilot/3`, `campus-pilot/4`, `campus-pilot/5`, and `campus-pilot/6` during rollout.
